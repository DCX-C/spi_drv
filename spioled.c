...
spidev_sync(struct spi_oled *oled, struct spi_message *message)
{
	...
	spin_lock_irq(&oled->spi_lock);
	spi = oled->spi;
	spin_unlock_irq(&oled->spi_lock);
...
spidev_sync_write(struct spi_oled *oled, size_t len)
{
	struct spi_transfer	t = {
			.tx_buf		= oled->tx_buffer,
			.len		= len,
...
static inline ssize_t
spidev_sync_read(struct spi_oled *oled, size_t len)
{
	struct spi_transfer	t = {
			.rx_buf		= oled->rx_buffer,
			.len		= len,
			.speed_hz	= oled->speed_hz,
		};
	struct spi_message	m;
...
spidev_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	status = spidev_sync_read(oled, count);
	if (status > 0) {
		unsigned long	missing;

		missing = copy_to_user(buf, oled->rx_buffer, status);
		if (missing == status)
			status = -EFAULT;
		else
		...
		struct spi_ioc_transfer *u_xfers, unsigned n_xfers)
{
	...
	for (n = n_xfers, k_tmp = k_xfers, u_tmp = u_xfers;
		...
		if (!k_tmp->speed_hz)
			k_tmp->speed_hz = oled->speed_hz;
#ifdef VERBOSE
		dev_dbg(&oled->spi->dev,
			"  xfer len %u %s%s%s%dbits %u usec %u usec %uHz\n",
			u_tmp->len,
			u_tmp->rx_buf ? "rx " : "",
			u_tmp->tx_buf ? "tx " : "",
			u_tmp->cs_change ? "cs " : "",
			u_tmp->bits_per_word ? : oled->spi->bits_per_word,
			u_tmp->delay_usecs,
			u_tmp->word_delay_usecs,
			u_tmp->speed_hz ? : oled->spi->max_speed_hz);
#endif
		spi_message_add_tail(k_tmp, &msg);
	}

	status = spidev_sync(oled, &msg);
	if (status < 0)
		goto done;
...
spidev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SPI_IOC_RD_MAX_SPEED_HZ:
		retval = put_user(oled->speed_hz, (__u32 __user *)arg);
		break;
	...
	default:
		...
		retval = spidev_message(oled, ioc, n_ioc);
		kfree(ioc);
		break;
	}
...
static int spidev_open(struct inode *inode, struct file *filp)
{
	...
	list_for_each_entry(oled, &device_list, device_entry) {
		if (oled->devt == inode->i_rdev) {
			status = 0;
			break;
		}
	}
	...
	if (!oled->tx_buffer) {
		oled->tx_buffer = kmalloc(bufsiz, GFP_KERNEL);
		if (!oled->tx_buffer) {
			dev_dbg(&oled->spi->dev, "open/ENOMEM\n");
			status = -ENOMEM;
			goto err_find_dev;
		}
	}
	...
	if (!oled->rx_buffer) {
		oled->rx_buffer = kmalloc(bufsiz, GFP_KERNEL);
		if (!oled->rx_buffer) {
			dev_dbg(&oled->spi->dev, "open/ENOMEM\n");
			status = -ENOMEM;
...
static int spidev_release(struct inode *inode, struct file *filp)
{
	...
	if (!oled->users) {
		...
		if (dofree)
			...
			oled->speed_hz = oled->spi->max_speed_hz;
	}
#ifdef CONFIG_SPI_SLAVE
	if (!dofree)
		spi_slave_abort(oled->spi);
#endif
	mutex_unlock(&device_list_lock);
...
static int spidev_probe(struct spi_device *spi)
{
	...
	oled->spi = spi;
	spin_lock_init(&oled->spi_lock);
	mutex_init(&oled->buf_lock);

	INIT_LIST_HEAD(&oled->device_entry);
	...
	if (minor < N_SPI_MINORS) {
		...
		oled->devt = MKDEV(SPIDEV_MAJOR, minor);
		dev = device_create(spidev_class, &spi->dev, oled->devt,
				    oled, "spioled%d.%d",
...
{
	struct spi_oled	*oled = spi_get_drvdata(spi);
	...
	list_del(&oled->device_entry);
	device_destroy(spidev_class, oled->devt);
	clear_bit(MINOR(oled->devt), minors);
	if (oled->users == 0)
		kfree(oled);
	mutex_unlock(&device_list_lock);
...
static int __init spi_oled_init(void)
{
	...
	status = register_chrdev(SPIDEV_MAJOR, "spi_oled", &spidev_fops);
	if (status < 0)
		return status;
